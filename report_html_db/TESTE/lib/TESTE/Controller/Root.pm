...
TESTE::Controller::Root - Root Controller for TESTE
...
sub searchDatabase :Path("SearchDatabase") :Args(0) {
	...
	$c->stash(template => 'teste/search-database/index.tt');
...
sub about :Path("About") :Args(0) {
	...
	$c->stash(template => 'teste/about/index.tt');
	$c->stash->{hadGlobal} = 0;
	$c->stash->{hadSearchDatabase} = 1;
...
sub blast :Path("Blast") :Args(0) {
	...
	$c->stash(template => 'teste/blast/index.tt');
	$c->stash->{hadGlobal} = 0;
	$c->stash->{hadSearchDatabase} = 1;
...
sub downloads :Path("Downloads") :Args(0) {
	...
	$c->stash(template => 'teste/downloads/index.tt');
	$c->stash->{hadGlobal} = 0;
	$c->stash->{hadSearchDatabase} = 1;
...
sub help :Path("Help") :Args(0) {
	...
	$c->stash(template => 'teste/help/index.tt');
	$c->stash->{hadGlobal} = 0;
	$c->stash->{hadSearchDatabase} = 1;
...
sub index :Path :Args(0) {
	...
	$c->stash(template => 'teste/home/index.tt');
	$c->stash->{hadGlobal} = 0;
	$c->stash->{hadSearchDatabase} = 1;
...
sub downloadFile : Path("DownloadFile") : CaptureArgs(1) {
	...
	my $filepath = $c->model('Basic::File')->(
		{
			tag => "$type"
		},
		{
