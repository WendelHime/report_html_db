...
sub geneByPosition_GET {
	...
	standardStatusOk( $self, $c,
		$c->model('Repository')->searchGene( \%hash ) );
}

sub targetClass : Path("/SearchDatabase/targetClass") : CaptureArgs(2) :
  ActionClass('REST') { }

sub targetClass_GET {
	my ( $self, $c, $type_id, $feature_id ) = @_;
	if ( !$type_id and defined $c->request->param("type_id") ) {
		$type_id = $c->request->param("type_id");
	}
	if ( !$feature_id and defined $c->request->param("feature_id") ) {
		$feature_id = $c->request->param("feature_id");
	}
	standardStatusOk( $self, $c,
		$c->model('Repository')->get_target_class( $type_id, $feature_id ) );
}
