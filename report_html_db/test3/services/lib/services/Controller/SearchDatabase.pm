...
sub geneByPosition_GET {
	...
	standardStatusOk( $self, $c, $c->model('Repository')->searchGene( \%hash ) );
}

sub targetClass : Path("/SearchDatabase/targetClass") : CaptureArgs(2) : ActionClass('REST') { }

sub targetClass_GET {
	my($self, $c, $type_id, $feature_id) = @_;
	standardStatusOk($self, $c, $c->model('Repository')->get_target_class($type_id, $feature_id));
}
